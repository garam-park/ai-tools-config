...
GitHub Copilot은 이 리포에서 **스킬만** 받고(`~/.agents/skills/`), 글로벌 지침은 받지 않는다.

- 리포 단위 지침 파일인 `.github/copilot-instructions.md`는 프로젝트별(per-project) 메커니즘이라 글로벌 지침의 대체재가 아니다.
- 사용자 전역(글로벌) 지침 메커니즘의 존재와 경로는 **미검증** 상태다. 공식 문서에서 확인한 뒤에만 설치기에 추가한다 (후속 작업 — [ARCHITECTURE-REVIEW.md](../ARCHITECTURE-REVIEW.md)의 후속 작업 목록 참조).
