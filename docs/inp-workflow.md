...
5. `inp-handle-pr`: 리뷰, CI, 후속 수정, merge readiness를 처리한다.
...
| 이미 열린 PR의 리뷰나 CI를 처리해야 한다 | `inp-handle-pr` | PR 메타데이터, 리뷰 스레드, 체크 결과, 로컬 상태 | 후속 커밋, push, PR 업데이트 |
...
### create-pr와 handle-pr의 경계

- PR이 아직 없거나 최초 설명이 비어 있으면 `inp-create-pr`를 쓴다.
- PR이 이미 있고 리뷰, CI, 후속 수정이 남아 있으면 `inp-handle-pr`를 쓴다.
...
- [skills/inp-handle-pr/SKILL.md](../skills/inp-handle-pr/SKILL.md)