...
> 작업 사본 조정: 리뷰 실행용 프레임워크 선택 기준을 추가하고, 특정 설계서 전용 매핑은 제거했다.
...
7종을 모두 적용하지 않는다. 문서가 실제로 다루는 범위에 해당하는 항목만 누락 후보를 찾고, 나머지는 `해당 없음`으로 남긴다.

| 문서 유형 | 우선 검토 | 보조 검토 |
| --- | --- | --- |
| 엔터프라이즈 전체 | TOGAF, IEEE 1471 / ISO 42010 | Kruchten 4+1, ADR |
| 클라우드 워크로드 | AWS Well-Architected | C4 Model, IEEE 1471 / ISO 42010, ADR |
| 애플리케이션·런타임 | 12-Factor App | C4 Model, ADR, 클라우드 문서면 AWS Well-Architected |
| 배포·인프라·네트워크 | TOGAF Technology 계층, Kruchten Process/Physical | 클라우드 워크로드면 AWS Well-Architected |
| 도식 중심 설계 | C4 Model | IEEE 1471 / ISO 42010 |
| 결정 기록 | ADR | 결정과 연결된 다른 프레임워크만 |
...
| --- | --- |
| Logical | 기능·책임 분리 |
| Development | 모듈·빌드 의존 |
| Process | 런타임 동시성 |
| Physical | 노드·배치 |
| Scenarios (+1) | 핵심 유스케이스로 다른 4개 뷰 검증 |
