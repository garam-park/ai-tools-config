...
- **비밀값 안전**: 커밋된 시크릿 없음. `ntn` 인증 정보와 `NOTION_API_TOKEN` 같은 환경변수는 리포 밖에서 주입, `.gitignore`가 `.env` 계열 차단.
...
| 스킬 | `skills/` 6종 (`ntn-*` 5 + `clarify`) | 양호 — 중립 SKILL.md + Codex 메타 | `skills/clarify/SKILL.md`에 구경로(`~/.local/share/skills/`) 서술 잔존 | 리포 직결 심링크 모델로 서술 수정 (완료) |
...
| 비밀값 | 커밋된 시크릿 없음, `ntn` 인증 정보와 `NOTION_API_TOKEN`은 리포 밖에서 관리, `.gitignore`에 `.env` 차단 | 양호 | 없음 | 현상 유지 + device-setup.md에 주입 안내 (완료) |
...
| stale 참조 수정 | `skills/clarify/SKILL.md`의 구 설치 경로 서술을 리포 직결 심링크 모델로 갱신 |
