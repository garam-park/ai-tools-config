...
- 사용자가 명시적으로 다른 도구를 지정하지 않는 한, 이 리포에서 정의한 `nt-*` 5종 스킬은 Copilot CLI 세션에서도 `~/.agents/skills/`를 통해 그대로 로드된다.
- 인라인 `@nt-<name>` 호출 외에 사용자가 `TSK-<id>` 형식의 작업을 지시하면 `nt-start-task` 스킬을 우선 고려한다.
