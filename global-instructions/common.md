...
- 작업 카드를 참조하는 경우 커밋 메시지에 다음 토큰을 포함한다:
  ...
  - 이 규약은 `git log --grep="task "` 로 작업 카드 ↔ 커밋 매핑 추적을 가능하게 한다
- 큰 변경은 작은 단계로 나눠서 진행한다

### 작업 카드 라이프사이클

- 새 작업은 `tasks/001.todo/total/NN-*.md` 로 생성하고 `## 상태` 체크박스로 진행한다
- 완료된 작업은 `git mv` 로 `tasks/002.done/total/` 로 이동한다
- 작업 시작/완료 시 `tasks/STATUS.md` 도 함께 갱신한다
